  ...
  <div class="header">
    <div class="left_content"><a href="index.htm"><img border="0" src="images/logo.gif" alt="" title="" /></a>
       ...
       <tr> 
          ...
          <form method="post" action="servlet/update1"><% 
   Bookinfo userid=(Bookinfo)session.getAttribute("userid");
    %>
    
     编号：<input type="text" name="id"  value="<%=userid.getId()%>"/>
   <p>&nbsp;书名：<input type="text" name="a"  value="<%=userid.getBookname()%>"></p>
   
   <p>&nbsp;作家：<input type="text" name="b"  value="<%=userid.getWriter() %>"></p>
   
   <p>&nbsp;出版社：<input type="text" name="c"  value="<%=userid.getPublisher() %>"></p>
   <p>&nbsp;价钱：<input type="text" name="d"  value="<%=userid.getPrice() %>"></p>
   <p>日期：<input type="text" value="<%=userid.getDate()%>"name="e"></p>
